 ...
 */
bdd_node_t *
bdd_init_node_ext(bdd_t *bdd, bdd_node_t *parent,
...
static void
dump_bdd_r(bdd_node_t *n, unsigned int level)
{
	if (n->iszero != NULL) {
#ifdef VERBOSE
		printf("(%s[%p] ? ", n->var, n);
		dump_bdd_r(n->isone, level+1);
		printf("[%p] : ", n->isone);
		dump_bdd_r(n->iszero, level+1);
		printf("[%p])", n->iszero);
#else
		printf("(%s ? ", n->var);
		dump_bdd_r(n->isone, level+1);
...
int
bdd_compact(bdd_t **bdd, int stats)
{
	int result;
	bdd_t *out;
	hash_t *h;
	...
	if (stats) {
		printf("Freeing unoptimized BDD... ");
		fflush(stdout);
	}
...
void
bdd_merge(bdd_t *bdd, bdd_node_t *n, bdd_t *subbdd)
{
	bdd_node_t *n2;
...
void
bdd_output_candidates(bdd_t *bdd, list_t *vars)
{
	listentry_t *e, *node;
...
bdd_output_candidate(bdd_node_t *n, list_t *vars)
{
	...
	while ((cur = n->parent) != NULL) {
		...
		if (cur->iszero == n) hash_put(used, cur->var, no.var, 2);
			...
			if (cur->isone == n) hash_put(used, cur->var, yes.var, 2);
			else abort();
		n = cur;
	}
